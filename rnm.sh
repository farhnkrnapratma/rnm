...
is_group_or_other_writable()
{
  ...
  if [[ -n $mode && ${mode: -1:1} =~ [2367] ]]; then
    return 0
  fi
  if [[ -n $mode && ${mode: -2:1} =~ [2367] ]]; then
    gid="$(stat -c '%g' -- "$path" 2> /dev/null || stat -f '%g' -- "$path" 2> /dev/null || printf '')"
    pgid="$(id -g 2> /dev/null || printf '')"
    if [[ -n $gid && -n $pgid && $gid -eq $pgid ]]; then
      return 1
    fi
    return 0
  fi
...
write_rollback_script()
{
  ...
  tmpfile="$(make_temp_file_in_dir "$state_dir" ".rnm-rollback.XXXXXX")"
  {
    ...
    printf '%s\n' '# Preflight every operation before moving anything.'
    printf '%s%s%s\n' 'path_exists() { [[ -e $' '1 || -L $' '1 ]]; }'
    printf '%s\n' 'srcs=()'
    printf '%s\n' 'tgts=()'
...
initial_source_for() {
	local path="$1"
	local upto="$2"
	local j src tgt suffix
	for ((j = upto - 1; j >= 0; j--)); do
		src="${srcs[$j]}"
		tgt="${tgts[$j]}"
		if [[ $path == "$tgt" ]]; then
			path="$src"
...
path_moves_away() {
	...
	local j initial
	for ((j = 0; j < ${#srcs[@]}; j++)); do
		initial="$(initial_source_for "${srcs[$j]}" "$j")"
		if [[ $path == "$initial" || $path == "$initial"/* ]]; then
			return 0
		fi
	done
	return 1
